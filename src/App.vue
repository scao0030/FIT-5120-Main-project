...
// Responsibilities:
//   1. Global language state — provides lang ref + setLang()
//      to all child components via Vue's provide/inject system
//   2. SPA page routing — swaps the active page component
//      based on the currentPage ref (no vue-router needed)
//   3. Animated background SVG — fixed behind all pages
//
// Note: the password gate has been removed in this version.
// Authentication is no longer required to access the app.
// ============================================================
...
// ── Language State ───────────────────────────────────────────
// Read the user's previously saved language preference from localStorage.
// Falls back to DEFAULT_LANG if the saved value is no longer in SUPPORTED_LANGUAGES
// (e.g. if a language was removed in a later version).
const savedLang = localStorage.getItem('preferred-lang')
const lang = ref(
  SUPPORTED_LANGUAGES.find((l) => l.code === savedLang) ? savedLang : DEFAULT_LANG
)

// setLang: switches the active language and persists the choice to localStorage
// so the preference survives page refreshes.
function setLang(code) { lang.value = code; localStorage.setItem('preferred-lang', code) }

// provide: makes lang (reactive ref) and setLang available to all descendant components.
// Child components access these via inject('lang') and inject('setLang').
// This avoids prop-drilling through every layer of the component tree.
provide('lang', lang)
provide('setLang', setLang)

// ── Page Routing ─────────────────────────────────────────────
// currentPage: the id of the currently displayed page.
// Set to 'home' on first load; updated by TopNav and page-level navigate events.
const currentPage = ref('home')

// currentView: resolves currentPage to its corresponding Vue component.
// Uses an object map for clean one-liner routing without if-else chains.
// Falls back to HomePage if the page id is not found (e.g. unknown navigate event).
...
<template>
  <!-- Decorative full-screen background: fixed behind all page content.
       aria-hidden="true" so screen readers skip the purely visual SVG. -->
  <div class="bg-scene" aria-hidden="true">
    <svg width="100%" height="100%" viewBox="0 0 1400 900" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- Cloud groups (overlapping ellipses give a fluffy appearance) -->
      ...
      <!-- Birds (simple curved stroke pairs) -->
      ...
      <!-- Ground / horizon gradient ellipses -->
      ...
      <!-- Trees left side (trunk + layered foliage ellipses) -->
      ...
      <!-- Person 1 (sitting, facing right, with glasses) -->
      ...
      <!-- Person 2 (standing, different skin tone, arm extended) -->
      ...
      <!-- Arm pointing left (helper gesture) -->
      ...
      <!-- Floating particles (CSS animated via SVG animate) -->
      <g class="particles">
  ...
  <div class="app-shell">
    <!-- Top navigation bar:
         nav-items    — list of page entries from siteContent.js
         current-page — highlights the active nav button
         @navigate    — updates currentPage when user clicks a nav button -->
    <TopNav
      :nav-items="navItems"
    ...
    <main class="page-shell">
      <!-- Dynamic component: renders the page matching currentView.
           Also listens for 'navigate' events emitted from within pages
           (e.g. a CTA button on HomePage that links to GuidesPage). -->
...
<style scoped>
/* Full-screen decorative background: fixed so it stays still while pages scroll.
