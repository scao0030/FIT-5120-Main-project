...
const routeSourceId = 'help-route'
const routeLayerId = 'help-route-line'
...
function createUserLocationMarkerElement() {
  ...
  element.setAttribute('aria-label', t(lang.value, 'help.myLocationLabel'))
  return element
}
...
function geolocationErrorMessage(error) {
  switch (error?.code) {
    case 1:
      return t(lang.value, 'help.locationDenied')
    case 2:
      return t(lang.value, 'help.locationUnavailable')
    case 3:
      return t(lang.value, 'help.locationTimeout')
    default:
      return t(lang.value, 'help.locationUnsupported')
  }
}
...
function requestUserLocation() {
  ...
  if (!navigator.geolocation) {
    locationError.value = t(lang.value, 'help.locationUnsupported')
    return Promise.reject(new Error(locationError.value))
  }
...
function toggleMapPanel() {
  isMapPanelCollapsed.value = !isMapPanelCollapsed.value
  nextTick(() => {
    map?.resize()
  })
}
...
function resetRouteMetrics() {
  routeMetricsByMode.value = {
    driving: {},
    walking: {},
  }
}

function getPlaceRouteMetrics(placeId, profile = routeProfile.value) {
...
function applyRouteMetricsToPlace(placeId, profile, route) {
  ...
  searchResults.value = searchResults.value.map((place) => {
    if (place.id !== placeId) return place

    const routeDistanceByMode = {
      ...(place.routeDistanceByMode || {}),
      [profile]: formattedDistance,
    }

    const routeDurationByMode = {
      ...(place.routeDurationByMode || {}),
      [profile]: formattedDuration,
    }
    ...
    return {
      ...
      routeDistanceByMode,
      routeDurationByMode,
      distance: profile === routeProfile.value ? formattedDistance : place.distance,
    }
  })
...
function formatDistanceKmValue(distanceMeters) {
  if (!Number.isFinite(distanceMeters)) return null
...
async function fetchRouteMatrix(origin, places, profile = routeProfile.value) {
  ...
  const batches = []
  for (let index = 0; index < places.length; index += 24) {
    batches.push(places.slice(index, index + 24))
  }
  ...
  for (const batch of batches) {
    const coordinates = [
      ...
      ...batch.map((place) => `${place.lng},${place.lat}`),
    ].join(';')

    const destinations = batch.map((_, index) => String(index + 1)).join(';')
    const url = new URL(`https://api.mapbox.com/directions-matrix/v1/mapbox/${profile}/${coordinates}`)
    ...
    if (!response.ok || payload?.code !== 'Ok') {
      throw new Error(payload?.message || t(lang.value, 'help.routeUnavailable'))
    }
...
async function updateVisiblePlaceDistances(profile = routeProfile.value) {
  ...
  const origin = userLocation.value || (await requestUserLocation())
  const metrics = await fetchRouteMatrix(origin, routablePlaces, profile)

  const metricsByPlaceId = Object.fromEntries(
    metrics.map((item) => [
      item.placeId,
      {
        distance: formatRouteDistance(item.distanceMeters),
        duration: formatRouteDuration(item.durationSeconds),
        distanceKm: formatDistanceKmValue(item.distanceMeters),
      },
    ]),
  )

  routeMetricsByMode.value[profile] = {
    ...routeMetricsByMode.value[profile],
    ...Object.fromEntries(
      Object.entries(metricsByPlaceId).map(([placeId, value]) => [
        placeId,
        { distance: value.distance, duration: value.duration },
      ]),
    ),
  }
  ...
  searchResults.value = searchResults.value
    .map((place) => {
      const routeMetrics = metricsByPlaceId[place.id]
      if (!routeMetrics) return place

      return {
        ...place,
        routeDistanceByMode: {
          ...(place.routeDistanceByMode || {}),
          [profile]: routeMetrics.distance,
        },
        routeDurationByMode: {
          ...(place.routeDurationByMode || {}),
          [profile]: routeMetrics.duration,
        },
        routeDistanceKmByMode: {
          ...(place.routeDistanceKmByMode || {}),
          [profile]: routeMetrics.distanceKm,
        },
        distance: profile === routeProfile.value ? routeMetrics.distance : place.distance,
        distanceKm: profile === routeProfile.value ? routeMetrics.distanceKm : place.distanceKm,
      }
    })
    .sort((a, b) => {
      const aDistance = a.routeDistanceKmByMode?.[profile]
      const bDistance = b.routeDistanceKmByMode?.[profile]
...
async function fetchRoute(origin, destination) {
  ...
  const url = new URL(`https://api.mapbox.com/directions/v5/mapbox/${routeProfile.value}/${coordinates}`)
  url.searchParams.set('access_token', mapboxToken)
  ...
  const response = await fetch(url)
  const payload = await response.json().catch(() => null)
  if (!response.ok || payload?.code !== 'Ok' || !payload?.routes?.[0]?.geometry) {
    throw new Error(payload?.message || t(lang.value, 'help.routeUnavailable'))
  }
...
async function showRouteToPlace(place) {
  ...
  try {
    ...
    const routeMetrics = applyRouteMetricsToPlace(place.id, routeProfile.value, route)
    routeDestinationId.value = place.id
    routeSummary.value = {
      distance: routeMetrics.distance,
      duration: routeMetrics.duration,
      destinationName: place.name,
      mode: routeProfile.value,
    }
    routeMetricsByMode.value[routeProfile.value] = {
      ...routeMetricsByMode.value[routeProfile.value],
      [place.id]: {
        distance: routeMetrics.distance,
        duration: routeMetrics.duration,
    ...
    searchResults.value = searchResults.value.map((item) =>
      item.id === place.id
        ? {
            ...item,
            routeDistanceKmByMode: {
              ...(item.routeDistanceKmByMode || {}),
              [routeProfile.value]: formatDistanceKmValue(route.distance),
            },
          }
        : item,
    )
    ...
    routeError.value = error.message || t(lang.value, 'help.routeUnavailable')
  }
}
...
async function onSearch() {
  ...
  try {
    ...
    try {
      await updateVisiblePlaceDistances(routeProfile.value)
    } catch (routeMetricsError) {
      console.error('[HelpPage] Route distance preload failed:', routeMetricsError)
    }
  } catch (err) {
...
function syncUserLocationMarker() {
  ...
  userLocationMarker.setLngLat([userLocation.value.lng, userLocation.value.lat]).addTo(map)
}
...
function updateMapViewport() {
  ...
  if (routeDestinationId.value && userLocation.value) {
    const destination = mapPlaces.value.find((place) => place.id === routeDestinationId.value)
    if (destination) {
      const bounds = new mapboxgl.LngLatBounds()
      bounds.extend([userLocation.value.lng, userLocation.value.lat])
      bounds.extend([destination.lng, destination.lat])
      map.fitBounds(bounds, { padding: 90, maxZoom: 13.5, duration: 700 })
      return
    }
  }

  const viewportPoints = mapPlaces.value.map((place) => [place.lng, place.lat])
  if (userLocation.value) {
    viewportPoints.push([userLocation.value.lng, userLocation.value.lat])
  }
...
watch(filteredPlaces, () => {
  if (routeDestinationId.value && !filteredPlaces.value.some((place) => place.id === routeDestinationId.value)) {
    clearRoute()
  }
  refreshMap()
})
...
watch(selectedPlace, (place) => {
  if (!map || !isMapReady.value) return
  syncMarkerSelection()
  if (routeDestinationId.value) return
...
watch(routeProfile, () => {
  if (hasSearched.value) {
    updateVisiblePlaceDistances(routeProfile.value).catch((error) => {
      console.error('[HelpPage] Route distance refresh failed:', error)
    })
  }

  if (!routeDestinationId.value) return
  const destination = filteredPlaces.value.find((place) => place.id === routeDestinationId.value)
  if (destination) {
    showRouteToPlace(destination)
  }
})
</script>
    ...
    <div class="container page-section">
      ...
      <section class="map-preview-section mb-4">
        <div class="map-preview-shell">
          <div class="map-preview-copy">
            <span class="map-preview-kicker">{{ t(lang, 'help.mapKicker') }}</span>
              ...
              <div>
                ...
                <p v-else-if="routeError" class="map-location-feedback error mt-3 mb-0" role="alert">
                  <IconGlyph name="warning" /> {{ routeError }}
                </p>
                <p v-else-if="userLocation" class="map-location-feedback success mt-3 mb-0">
          ...
          <div class="map-layout" :class="{ 'panel-collapsed': isMapPanelCollapsed }">
            <div class="map-canvas-frame">
              <div class="map-canvas">
                <div ref="mapContainer" class="real-map"></div>

                <div v-if="hasSearched && !nearbyPlaces.length" class="map-empty-card">
            ...
            <aside v-if="hasSearched && filteredPlaces.length" class="map-side-panel" :class="{ collapsed: isMapPanelCollapsed }">
              ...
              <div v-if="!isMapPanelCollapsed" class="map-side-content">
                ...
                <div class="route-mode-panel">
                  <div class="route-summary-title">{{ t(lang, 'help.routeModeTitle') }}</div>
                  <div class="route-mode-switch">
                    <button
                      ...
                      :class="{ active: routeProfile === option.id }"
                      @click="routeProfile = option.id"
                    >
                      {{ option.label }}
                    </button>
                  </div>
                ...
                <div v-if="routeSummary" class="route-summary-card">
                  <div class="route-summary-title">{{ t(lang, 'help.routeTitle') }}</div>
                  <p class="mb-2">{{ t(lang, 'help.routeTo') }} <strong>{{ routeSummary.destinationName }}</strong></p>
                  <p class="route-mode-copy mb-2">{{ routeProfile === 'walking' ? t(lang, 'help.walkingMode') : t(lang, 'help.drivingMode') }}</p>
                  <div class="route-summary-metrics">
                    <span v-if="routeSummary.duration">{{ routeSummary.duration }}</span>
                    <span v-if="routeSummary.distance">{{ routeSummary.distance }}</span>
                  </div>
                </div>
                ...
                <div v-if="selectedPlace" class="map-panel-card">
                  <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
                    ...
                    <span v-if="displayedDistance(selectedPlace)" class="badge rounded-pill text-bg-primary">{{ displayedDistance(selectedPlace) }}</span>
                  </div>
                  ...
                  <div class="map-panel-actions">
                    <button type="button" class="btn btn-primary btn-lg" @click="showRouteToPlace(selectedPlace)">
                      {{ routeDestinationId === selectedPlace.id ? t(lang, 'help.routeShown') : t(lang, 'help.getDirections') }}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <article class="card soft-card place-card h-100" :class="{ selected: selectedPlaceId === place.id }">
              <div class="card-header place-card-header p-4">
                <div class="d-flex justify-content-between align-items-start gap-3">
                  ...
                  <span v-if="displayedDistance(place)" class="distance-badge">{{ displayedDistance(place) }}</span>
                </div>
              </div>
              ...
              <div class="card-body p-4 d-flex flex-column">
                ...
                <div class="card-actions mt-auto">
                  ...
                  <button type="button" class="btn btn-primary btn-lg" @click="showRouteToPlace(place)">
                    {{ routeDestinationId === place.id ? t(lang, 'help.routeShown') : t(lang, 'help.getDirections') }}
                  </button>
                </div>
              </div>
            </article>
...
.route-summary-card {
  ...
  border: 1px solid rgba(23, 89, 215, 0.18);
  border-radius: 20px;
...
.map-location-feedback.error {
  color: #b3261e;
  font-weight: 600;
}

.map-location-feedback.success {
  color: #1d8f63;
  font-weight: 600;
}
...
.map-side-toggle {
  ...
  min-height: 2.6rem;
  padding: 0.45rem 0.85rem;
  border: 1px solid rgba(99, 124, 190, 0.18);
  border-radius: 999px;
...
.map-side-panel.collapsed .map-side-toggle {
...
.route-mode-btn.active {
  ...
  background: #1759d7;
  color: #fff;
  box-shadow: 0 10px 22px rgba(23, 89, 215, 0.16);
}
...
:deep(.mapbox-user-marker) {
  ...
  box-shadow:
    0 0 0 7px rgba(23, 89, 215, 0.2),
    0 10px 24px rgba(29, 53, 110, 0.22);
}

:deep(.mapbox-user-marker span) {
  position: absolute;
